...
def Absolute_Signal_to_noise_Ratio(arr):
    ...
    ----------
    arr
        numpy array of the filtered signal.
...
def Cardiac_Pulse_Power(arr, peaks, fs, each_slice_length=100):
    ...
    fs : int
        Sampling frequency of the signal, used to define the default slice length.
    each_slice_length : int
        Number of samples to include in each heart cycle slice. Default is 100 samples.

    Returns
    -------
    average_energy : float
        The average energy of the extracted heart cycles.
    """
...
def Kurtosis_Index(arr):
    ...
    ----------
    arr
        The input array for which to calculate the kurtosis.
    
    Returns
    -------
    kurtosis_index : float
        The kurtosis index of the array.
    """
...
def Signal_Consistency_Range(arr):
    ...
    ----------
    arr
        The input array for which to calculate the IQR.
...
def Clipping_Quality_Index(arr, locs, fs, threshold=1):
    ...
    locs : list of integers
        The indices of peaks around which waveforms are extracted.
    fs : int
        The sampling frequency of the signal.
    threshold : int or float
        The value above which the difference between consecutive 
        samples is considered to indicate clipping.

    Returns
    -------
    float
        The average proportion of non-clipped segments across all analyzed waveforms.
    """
...
def Relative_Power_SQI(arr, fs):
    ...
    ----------
    arr : np.ndarray
        The input signal from which to calculate RSQI.
    fs : int
        The sampling frequency of the signal.
    ...
    -------
    rsqi : float
        The calculated RSQI, representing the ratio of power in the target band to the broader band.
    """
...
def Std_of_PSD(arr, fs):
    ...
    arr
        The input array.
    fs
        The sampling frequency of the array. Default is 1.0.

    Returns
    -------
    std_psd
        The standard deviation of the PSD.
    """
...
def Dynamic_Correlation_Index(arr, locs, fs):
    ...
    average_quality : float
        Average quality index across all analyzed waveforms, based on their correlation to reference waveforms.
    """
